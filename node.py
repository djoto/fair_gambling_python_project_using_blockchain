    ...
    def connect_with_node(self, host, port, reconnect=False):
        ...
        for node in self.nodes_outbound:
            if node.host == host and node.port == port:
                print("connect_with_node: Already connected with this node (" + str(node.port) + ").")
                return True
        ...
        try:
            ...
            addrStr = str(self.host)+' '+str(self.port)+' '+str(self.id)
            sock.send(addrStr.encode('utf-8')) # Send my info to the connected node!
            ...
            for node in self.nodes_inbound:
                if node.host == host and node.id == connected_node_id:
                    print("connect_with_node: This node (" + str(node.port) + ") is already connected with us.")
                    return True
        ...
        while not self.terminate_flag.is_set():  # Check whether the thread needs to be closed
            try:
                ...
                if self.max_connections == 0 or len(self.nodes_inbound) < self.max_connections:
                    ...
                    # Basic information exchange (not secure) of the id's of the nodes!
                    connected_node_str = connection.recv(4096).decode('utf-8') # When a node is connected, it sends it info!
                    connected_node_arr = (connected_node_str.strip()).split(' ')
                    connected_node_host = connected_node_arr[0]
                    connected_node_port = int(connected_node_arr[1])
                    connected_node_id = connected_node_arr[2]
                    connection.send(self.id.encode('utf-8')) # Send my id to the connected node!

                    thread_client = self.create_new_connection(connection, connected_node_id, connected_node_host, connected_node_port)
                    thread_client.start()
